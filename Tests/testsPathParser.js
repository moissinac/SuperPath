...
QUnit.module("Parsing and reversing path");
...
QUnit.test( "Test parsing path data with absolute arc, and reversing it", function( assert ) {
    var data = "M100 100 A 100 60 15 0 0 300 200";
    var cmdList = pathparser.svg_parse_path(data);
    var relCmdList = pathparser.fullrelativePathCmdList(cmdList);
    var revCmdList = relCmdList.reverse();
    var strCmdList = revCmdList.toString(cmdList);
    assert.equal( strCmdList, "a100 60 15 0 1 -200,-100M-100,-100", "Passed!");
});
QUnit.test( "Test parsing path data with relative arc and reversing it", function( assert ) {
    var data = "M400 100 a 100 60 15 0 0 200 100";
    var cmdList = pathparser.svg_parse_path(data);
    var strCmdList = cmdList.toString(cmdList);
    var relCmdList = pathparser.fullrelativePathCmdList(cmdList);
